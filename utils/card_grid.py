...
def make_hover_image(page):
    hover_image = html.Div([
        html.Img(src=dash.get_asset_url(page["image"]), className="img-fluid hover-image",),
        html.Div(dbc.Button("See app", color="secondary"), className="hover-overlay",),
...
def make_card(page):
    ...
    card = dbc.Card([
            dbc.CardHeader([
                dbc.NavLink(page["title"], href=page["relative_path"],),
            ]),
            dbc.CardBody([
                html.A(make_hover_image(page), href=page["relative_path"], id=tooltip_id,),
                html.P(page["description"], className="card-text p-2 border-top",),
            ],),
